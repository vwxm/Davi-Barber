...
const emptyForm: FormState = { date: '', date_end: '', full_day: true, start_time: '', end_time: '', reason: '' }
...
export function BloqueiosManager({ blocks: initial }: BloqueiosManagerProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      setError(null)
      const isPeriod = !!form.date_end
      const result = await createBlock({
        date: form.date,
        date_end: isPeriod ? form.date_end : undefined,
        full_day: isPeriod ? true : form.full_day,
        start_time: isPeriod || form.full_day ? undefined : form.start_time || undefined,
        end_time: isPeriod || form.full_day ? undefined : form.end_time || undefined,
    ...
    <div className="space-y-4">
      ...
      {adding && (
        <form onSubmit={handleSubmit} className="bg-zinc-800 rounded-xl p-4 space-y-3">
          ...
          <Input label={form.date_end ? 'Data início' : 'Data'} name="date" type="date" value={form.date} onChange={handleChange} required />
          <Input label="Data fim (opcional — para período)" name="date_end" type="date" value={form.date_end} onChange={handleChange} min={form.date || undefined} />
          {form.date_end ? (
            <p className="text-zinc-400 text-xs">Período é sempre dia inteiro.</p>
          ) : (
            <>
          ...
          <div key={block.id} className="bg-zinc-800 rounded-xl p-4 flex items-center justify-between gap-3">
            <div className="flex-1 min-w-0">
              <p className="font-medium text-white">
                {block.date_end
                  ? `${formatDateBR(block.date)} – ${formatDateBR(block.date_end)}`
